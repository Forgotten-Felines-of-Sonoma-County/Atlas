...
async function main() {
  ...
  console.log(`\n${bold}ClinicHQ Cat Info Ingest${reset}`);
  ...
  let xlsxPath = options.xlsxPath || path.join(DEFAULT_INGEST_PATH, 'clinichq', options.date, 'cat_info.xlsx');
  ...
  if (!options.dryRun) {
    ...
    runner = new IngestRunner(client, SOURCE_SYSTEM, SOURCE_TABLE, {
      ...
      detectSuspect: detectBaseSuspectIssues,
    });
