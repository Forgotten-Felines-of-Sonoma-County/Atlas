#!/usr/bin/env node
/**
 * etapestry_mailchimp_export_csv.mjs - Ingests E-Tapestry Mailchimp export CSV
 */
import fs from 'fs';
...
const SOURCE_SYSTEM = 'etapestry';
const SOURCE_TABLE = 'mailchimp_export';
const DEFAULT_INGEST_PATH = process.env.LOCAL_INGEST_PATH || '/Users/benmisdiaz/Desktop/AI_Ingest';
const DEFAULT_DATE = '2026-01-09';
const ID_FIELD_CANDIDATES = ['Account Number', 'Constituent ID', 'Email', 'ID'];
...
function findFile(dateDir) {
  ...
  for (const f of fs.readdirSync(dateDir)) {
    if (/^etapestry_mailchimp.*\.csv$/i.test(f)) return path.join(dateDir, f);
  }
  return null;
}
...
function detectSuspectIssues(row) {
  ...
  const address = row['Address'] || row['Street'] || row['Mailing Address'] || '';
...
async function main() {
  ...
  console.log(`\n${bold}E-Tapestry Mailchimp Export Ingest${reset}`);
  if (!process.env.DATABASE_URL) { console.error(`${red}Error:${reset} DATABASE_URL not set`); process.exit(1); }

  let csvPath = options.csvPath || findFile(path.join(DEFAULT_INGEST_PATH, 'etapestry', options.date));
  if (!csvPath || !fs.existsSync(csvPath)) {
    console.log(`${yellow}SKIP:${reset} No etapestry_mailchimp file found`);
    process.exit(0);
  }
