#!/usr/bin/env node
/**
 * petlink_pets_xls.mjs - Ingests PetLink pet list XLS/XLSX
 * Note: xlsx lib can often read .xls files
 */
import fs from 'fs';
...
function findFile(dateDir) {
  ...
  for (const f of fs.readdirSync(dateDir)) {
    if (/^petlink_pet.*\.(xls|xlsx)$/i.test(f)) return path.join(dateDir, f);
  }
  return null;
}
...
async function main() {
  ...
  console.log(`\n${bold}PetLink Pets Ingest${reset}`);
  ...
  if (!xlsPath || !fs.existsSync(xlsPath)) {
    console.log(`${yellow}SKIP:${reset} No petlink_pets file found`);
    process.exit(0);
  }

  // Check if xlsx lib can read this file
