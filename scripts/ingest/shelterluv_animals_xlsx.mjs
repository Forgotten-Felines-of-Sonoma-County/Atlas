#!/usr/bin/env node
/**
 * shelterluv_animals_xlsx.mjs - Ingests Shelterluv animals XLSX
 */
import fs from 'fs';
...
const ID_FIELD_CANDIDATES = ['Internal-ID', 'Animal ID', 'ID'];
...
function findFile(dateDir) {
  ...
  for (const f of fs.readdirSync(dateDir)) {
    if (/^shelterluv_animals.*\.xlsx$/i.test(f)) return path.join(dateDir, f);
  }
  return null;
}
...
async function main() {
  ...
  console.log(`\n${bold}Shelterluv Animals Ingest${reset}`);
  ...
  if (!xlsxPath || !fs.existsSync(xlsxPath)) {
    console.log(`${yellow}SKIP:${reset} No shelterluv_animals file found`);
    process.exit(0);
  }

  const { headers, rows } = parseXlsxFile(xlsxPath);
