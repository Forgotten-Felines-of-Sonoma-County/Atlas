#!/usr/bin/env node
/**
 * shelterluv_people_xlsx.mjs - Ingests Shelterluv people XLSX
 */
import fs from 'fs';
...
function findFile(dateDir) {
  ...
  for (const f of fs.readdirSync(dateDir)) {
    if (/^shelterluv_people.*\.xlsx$/i.test(f)) return path.join(dateDir, f);
  }
  return null;
}
...
async function main() {
  ...
  console.log(`\n${bold}Shelterluv People Ingest${reset}`);
  ...
  if (!xlsxPath || !fs.existsSync(xlsxPath)) {
    console.log(`${yellow}SKIP:${reset} No shelterluv_people file found`);
    process.exit(0);
  }
