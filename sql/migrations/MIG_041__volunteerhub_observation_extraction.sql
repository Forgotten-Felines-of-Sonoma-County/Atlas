-- MIG_041__volunteerhub_observation_extraction.sql
-- Improved observation extraction with VolunteerHub support
--
-- Purpose:
--   Fix observation extraction to properly combine first/last name fields
--   for sources that use split name fields (VolunteerHub, Shelterluv, etc.)
--
-- APPLY:
--   psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f sql/migrations/MIG_041__volunteerhub_observation_extraction.sql

\echo '============================================'
\echo 'MIG_041: VolunteerHub Observation Extraction'
\echo '============================================'

-- ============================================
-- PART 1: Smart observation extraction with name combining
-- ============================================
\echo ''
\echo 'Creating extract_observations_from_staged_v2...'
...
-- ============================================
-- PART 2: Update main extraction to use v2
-- ============================================
\echo ''
\echo 'Updating main extraction function...'
...
BEGIN
    -- Delegate to v2 which handles name combining
    RETURN QUERY SELECT * FROM trapper.extract_observations_from_staged_v2(p_staged_record_id);
END;
$$ LANGUAGE plpgsql STABLE;

COMMENT ON FUNCTION trapper.extract_observations_from_staged IS
'Extracts observations from a staged record. Combines first/last name fields into full names.
Supports: Airtable, ClinicHQ, VolunteerHub, Shelterluv, PetLink.';
...
\echo ''
\echo '============================================'
\echo 'MIG_041 Complete'
\echo '============================================'

\echo ''
\echo 'Testing VolunteerHub extraction:'
SELECT *
FROM trapper.extract_observations_from_staged(
    (SELECT id FROM trapper.staged_records WHERE source_system = 'volunteerhub' LIMIT 1)
);

\echo ''
\echo 'To process VolunteerHub data:'
\echo '  SELECT trapper.populate_observations_for_latest_run(''users'');'
\echo '  SELECT * FROM trapper.upsert_people_from_observations(''users'');'
\echo '  SELECT trapper.populate_aliases_from_name_signals(''users'');'
